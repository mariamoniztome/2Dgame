export const SPELLS = {
  brisa_molhada: {
    id: 'brisa_molhada',
    name: 'Humidaris',
    plants: ['ventoinha', 'gotateia'],
    effect: 'capture_farfalha',
    description: 'Este feitiço diminui o fogo da Farfalha por algum tempo para que a consigas apanhar.',
    color: 0x4fc3f7,
    textureKey: 'spell_brisa',
    unlockZone: 1,
  },
  flutueminem: {
    id: 'flutueminem',
    name: 'Flutueminem',
    plants: ['ventoinha', 'bocarra'],
    effect: 'fly',
    description: 'Este feitiço dá-te o poder de voar durante 9 segundos. Protege-te de fendas, areia e água — mas no Planalto atrai o Furacão!',
    color: 0xd0e8f0,
    textureKey: 'spell_brisa',
    unlockZone: 2,
  },
  fogo_controlado: {
    id: 'fogo_controlado',
    name: 'Ignicura',
    plants: ['trepadeira', 'tezaluz'],
    effect: 'repel_creature',
    description: 'Este feitiço permite-te lançar uma pequena chama que espanta o Sussurro-ladrão.',
    color: 0xff7043,
    textureKey: 'spell_raiz',
    unlockZone: 2,
  },
  memoria_solo: {
    id: 'memoria_solo',
    name: 'Terramemoria',
    plants: ['espinhosa_doce', 'craveira'],
    effect: 'reveal_path',
    description: 'Este feitiço guia-te até ao Pântano através de pegadas que vão aparecendo no chão.',
    color: 0x66bb6a,
    textureKey: 'spell_canto',
    unlockZone: 2,
  },
  raiz_ardente: {
    id: 'raiz_ardente',
    name: 'Radixignis',
    plants: ['faisca_mato'],
    effect: 'clear_ground',
    description: 'Este feitiço faz com que uma raíz de fogo surja da terra durante 30 segundos, limpando a Areia Movediça e revelando a Sombravinha que se esconde sob ela.',
    color: 0xff7043,
    textureKey: 'spell_raiz',
    unlockZone: 3,
  },
  canto_jardim: {
    id: 'canto_jardim',
    name: 'Horticantus',
    plants: ['ninfaria'],
    effect: 'reveal_plants',
    description: 'Este feitiço faz com que todas as plantas fiquem visíveis no mapa durante 30 segundos.',
    color: 0x66bb6a,
    textureKey: 'spell_canto',
    unlockZone: 2,
  },
  espelho_memoria: {
    id: 'espelho_memoria',
    name: 'Memoriaspeculum',
    plants: ['ninfaria', 'sussurreira'],
    effect: 'open_vale_espelhos',
    description: 'Este feitiço abre-te o caminho para o Vale dos Espelhos.',
    color: 0xa8e07e,
    textureKey: 'spell_passo',
    unlockZone: 3,
  },
  Ancestria: {
    id: 'Ancestria',
    name: 'Ancestria',
    plants: ['lunaria_negra'],
    effect: 'defeat_ecos',
    description: 'Este feitiço lança um feixe de luz de forma circular à tua volta, derrotando os Ecos.',
    color: 0xa8e07e,
    textureKey: 'spell_passo',
    unlockZone: 3,
  },
};

export const SPELL_ORDER = [
  'brisa_molhada',    // 1. Humidaris       — ventoinha + gotateia (Zona 1)
  'fogo_controlado',  // 2. Ignicura         — trepadeira + tezaluz (Zona 2 Planície)
  'flutueminem',      // 3. Flutueminem      — ventoinha + bocarra  (Zona 2 Jardim)
  'memoria_solo',     // 4. Terramemoria     — espinhosa + craveira (Zona 2 Jardim)
  'canto_jardim',     // 5. Horticantus      — ninfaria alone       (Zona 2 Pântano)
  'raiz_ardente',     // 6. Radixignis       — faisca_mato alone    (Zona 3)
  'espelho_memoria',  // 7. Memoriaspeculum  — ninfaria + sussurreira (Zona 3)
  'Ancestria',        // 8. Ancestria        — lunaria_negra alone  (Zona 3)
];